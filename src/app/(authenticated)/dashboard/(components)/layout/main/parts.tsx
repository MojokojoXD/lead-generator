...
import { useTaskQueue } from '@/app/(authenticated)/dashboard/(components)/hooks/useTaskQueue';
import { useDashboardPortal } from '../../hooks/useDashboardPortal';
import { cn } from '@/lib/utils';
import { ArrowLeft, MenuIcon, Power, RotateCw } from 'lucide-react';
...
function Main( { children }: MainProps )
{
  ...
  const queueParams = useTaskQueue( { shouldProcess: true } );
    ...
    <DashboardContext.Provider value={ {
      ...
      <main className={ cn( 'fixed h-screen inset-y-0 bg-secondary pr-1.5 sm:transition-[left] ease-in-out duration-300',
        ...
        <section className='relative h-full w-dwv border bg-white rounded-2xl overflow-hidden'>
          ...
          <div className={ cn( 'absolute top-0 right-0 h-40 w-[20vw] flex justify-center items-center', queueParams.isProcessing ? 'z-10' : '-z-10' ) }>
            <span className='flex items-center justify-center h-10 aspect-square'>
