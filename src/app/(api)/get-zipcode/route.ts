...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!lat || !lng)
      return NextResponse.json({ message: 'invalid search params' }, { status: 400, statusText: 'invalid search params' });

    lat = parseFloat( lat );
    lng = parseFloat( lng );

    if ( Number.isNaN( lat ) || Number.isNaN( lng ) )
      return NextResponse.json( { message: 'invalid coordinate values' }, { status: 400, statusText: 'invalid coordinates' } );
    ...
    const cursor = await collection.find(
      ...
      } ).project( { _id: 0, location: 0 }).limit( 1 )
